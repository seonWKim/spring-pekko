...
class ClusterListenerSingletonBehavior implements SingletonBehavior<MemberEvent> {
    ...
    public Behavior<MemberEvent> create() {
        return Behaviors.setup(context -> {
            ...
            context.getLog().debug("Cluster Listener initialized on node: {}", cluster.selfMember().address());
            ...
            return Behaviors.receiveMessage(event -> {
                if (event instanceof ClusterEvent.MemberUp) {
                    ...
                    context.getLog().debug("🚀 New node joined the cluster: {}", memberUp.member().address());
                    ...
                    context.getLog().debug("❌ Node left the cluster: {} (was {})",
                    ...
                    context.getLog().debug("⚠️ Node is weakly up: {}", memberWeaklyUp.member().address());
                } else {
